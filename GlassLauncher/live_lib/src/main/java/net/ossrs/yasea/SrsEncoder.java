...
public class SrsEncoder {
    ...
    private boolean isAddPpsFrame = false;
    ...
    public void setPpsFrame(boolean isAdd) {
        this.isAddPpsFrame = isAdd;
    }

    public boolean start() {
        iswork = true;
        Log.i(TAG, "start: iswork is true");
        if (mp4Muxer == null) {
            return false;
        }
...
//        videoFlvTrack = flvMuxer.addTrack(videoFormat);
    ...
    public void setVideoHHDMode() {
        vBitrate = 2560 * 1000;  // 2560 kbps
        x264Preset = "fast";
    }

    public void setVideoHDMode() {
        vBitrate = 2000 * 1000;  // 1200 kbps
        x264Preset = "veryfast";
    }
...
//                throw new AssertionError("MTK encoding revolution stride must be 32x");
    ...
    private void onProcessedYuvFrame(byte[] yuvFrame, long pts) {
        ...
        for (; ; ) {
            int outBufferIndex = vencoder.dequeueOutputBuffer(vebi, 1000);
            if (outBufferIndex >= 0) {
    ...
    private void onEncodedAnnexbFrame(ByteBuffer es, MediaCodec.BufferInfo bi) {
        ...
        ByteBuffer bf = record.slice();
        byte[] dst = new byte[bf.capacity()];
        es.get(dst, 0, dst.length);

        //add sps*pps key before primary key
//        if (isAddPpsFrame) {
//            //添加参数帧 记录pps和sps
//            if (dst[0] == 0 && dst[1] == 0 && dst[2] == 0 && dst[3] == 1 && dst[4] == 103) {
//                sps = dst;
//            } else if (dst[0] == 0 && dst[1] == 0 && dst[2] == 0 && dst[3] == 1 && dst[4] == 101) {
//                //在关键帧前面加上pps和sps数据
//                mHandler.onSetStreamFrame(sps);
//                mHandler.onSetStreamFrame(dst);
//            } else {
//                mHandler.onSetStreamFrame(dst);
//            }
//        } else {
    ...
    private byte[] hwPortraitYuvFrame(byte[] data) {
        if (mCameraFaceFront) {
            switch (mVideoColorFormat) {
                ...
                // Changed by zhangsong , the color format is COLOR_TI_FormatYUV420PackedSemiPlanar in c100
...
//        vmci = chooseVideoEncoder("google");
        ...
        for (int i = 0; i < cc.colorFormats.length; i++) {
            ...
            //modified by yaochuanqi for c100 crash
            //if (cf >= cc.COLOR_FormatYUV420Planar && cf <= cc.COLOR_FormatYUV420SemiPlanar) {
            if ((cf >= cc.COLOR_FormatYUV420Planar && cf <= cc.COLOR_FormatYUV420SemiPlanar)
                    || (cf == cc.COLOR_TI_FormatYUV420PackedSemiPlanar)) {
