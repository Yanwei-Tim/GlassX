...
public class SrsCameraView extends SurfaceView implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    public interface PreviewCallback {
        void onGetYuvFrame(byte[] data, Camera camera);
    }
    ...
    public boolean startCamera() {
        ...
        Log.i("SrsEncoder", "startCamera: range[0]" + range[0]);
        Log.i("SrsEncoder", "startCamera: range[1]" + range[1]);
...
//        mCamera.setPreviewCallback(this);
    ...
    private int[] findClosestFpsRange(int expectedFps, List<int[]> fpsRanges) {
        ...
        Log.i("SrsEncoder", "findClosestFpsRange: suport measure:" + measure);

        for (int[] range : fpsRanges) {
            if (range[0] <= expectedFps && range[1] >= expectedFps) {
                int curMeasure = Math.abs(range[0] - expectedFps) + Math.abs(range[1] - expectedFps);
                Log.i("SrsEncoder", "findClosestFpsRange: suport curMeasure:" + curMeasure);
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        mPrevCb.onGetYuvFrame(data, camera);
